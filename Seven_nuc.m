...
if (read_from_file_flag==1)
    load('Seven_spin_DD_cross_corr_relmat_22ps/3.3Gs.mat');    
end
...
for p = 1:length(tau)
    ...
    for l = 1:NB
        ...
        if (D_D_corr_flag==1)
            ...
            R_DD_cross = Rrf;
            save(['Seven_spin_DD_cross_corr_relmat_22ps/' num2str(log(B(l))/log(10),3) 'Gs.mat'], "R_DD_cross", '-v7.3');
        end
        if (D_D_corr_flag==1)&&(read_from_file_flag==1)
            Rrf = R_DD_cross;
        end
        %% CSA
        if (CSA_flag==1)
